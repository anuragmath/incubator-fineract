...
public class Loan extends AbstractPersistable<Long> {
    ...
    public LoanRepaymentScheduleInstallment fetchRepaymentScheduleInstallment(final LocalDate transactionDate) {
        LoanRepaymentScheduleInstallment installment = null;
        if (transactionDate == null) { return installment; }
        for (final LoanRepaymentScheduleInstallment scheduleInstallment : this.repaymentScheduleInstallments) {
    ...
    private Money[] fetchInterestFeeAndPenaltyTillDate(final LocalDate paymentDate, final MonetaryCurrency currency,
        ...
        for (LoanCharge loanCharge : this.charges) {
            if (loanCharge.isActive() && loanCharge.isDueForCollectionFromAndUpToAndIncluding(installment.getFromDate(), paymentDate)) {
                if (loanCharge.isPenaltyCharge()) {
                    penaltyForCurrentPeriod = loanCharge.getAmount(getCurrency());
                    penaltyAccoutedForCurrentPeriod = loanCharge.getAmountWaived(getCurrency())
